...
class MLkNN(MLClassifierBase):
    ...
    def compute_prior(self, y):
        prior_prob_true = \
        np.array((self.s + y.sum(axis=0)) / (self.s * 2 + self.num_instances))[
            0]
    ...
    def compute_cond(self, X, y):
        if sparse.issparse(X):
            self.knn = NearestNeighbors(self.k, algorithm='brute').fit(X)
        else:
        ...
        for label in range(self.num_labels):
            for neighbor in range(self.k + 1):
                cond_prob_true[label, neighbor] = (self.s + c[
                    label, neighbor]) / (
                                                          self.s * (
                                                              self.k + 1) +
                                                          c_sum[label, 0])
                cond_prob_false[label, neighbor] = (self.s + cn[
                    label, neighbor]) / (
                                                           self.s * (
                                                               self.k + 1) +
                                                           cn_sum[label, 0])
    ...
    def fit(self, X, y):
        ...
        self.cond_prob_true, self.cond_prob_false = self.compute_cond(X,
                                                                      self.train_labels)
        return self
